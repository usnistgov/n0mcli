...
typedef struct {
	...
	LP_SEL_SET pidatasrc ;		/* ptr to Interval data source for each channel [nchnl] */
	INT_FMT_CDE *pint_fmt_cde ;
	SCALAR_SET pscalar ;	/* Scalars applied to interval data on a per-channel basis */
	DIVISOR_SET pdivisor ;	/* Divisors applied to interval data on a per-channel basis */
